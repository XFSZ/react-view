...
import { TLayerDataListDefaultType, TLayerDataListDefaultTypeItem } from '../FormEditor/types';
  ...
  return connectDropTarget(
    ...
    connectDragPreview(
      ...
      <div className={styles.listItem} style={Object.assign({}, { opacity })}>
        <div className={styles.tit}>层级：{zIndex}</div>
        <div className={styles.tit}>状态： {visibility===1 ? "显示" :"隐藏"}</div>
...
type DndItemProps = TLayerDataListDefaultTypeItem & {
  onDel: Function;
  onEdit: Function;
  key: number;
  find: Function;
...
export type LayerDataListMemo = {
  onChange?: (v: TLayerDataListDefaultType) => void;
  value?: TLayerDataListDefaultType;
 // cropRate: number;
};

export type LayerDataListType = LayerDataListMemo & {
  connectDropTarget: ConnectDropTarget;
};

const List = function(props: LayerDataListType) {
  const { onChange, value, connectDropTarget } = props;
  const [list, setList] = useState(value);
  const [visible, setVisible] = useState(false);
  const [curItem, setCurItem] = useState<TLayerDataListDefaultTypeItem>();
  ...
  const handleEdit = useCallback((item: TLayerDataListDefaultTypeItem) => {
    console.log('b');
    setVisible(true);
    setCurItem(item);
  }, []);
...
export default memo((props: LayerDataListMemo) => {
  return (
